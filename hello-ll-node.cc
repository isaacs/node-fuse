...
static Handle<Value> Mount (const Arguments& args) {
  ...
  if (args.Length() == 1) {
    ...
    fuseArgs = Handle<Array>(Array::Cast(*args[1]));
  }
  ...
  argv[0] = (char *)calloc(sizeof(char), mp.length() + 1);
  assert(argv[0]);
  strncpy(argv[0], *mp, mp.length() + 1);
  for (int i = 1; i < argc; i ++) {
    String::Utf8Value fa(fuseArgs->Get(i - 1)->ToString());
    argv[i] = (char *)calloc(sizeof(char), fa.length() + 1);
    assert(argv[i]);
